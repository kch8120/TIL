...
당신이 만야 숫자를 넣을 수 있는 변수를 만든다 하자.<br>
그럴 때 당신은 정수/실수, 수의 범위 등을 고려해야 한다.<br>
정수와 실수의 데이터형은 서로 다르며, 수의 범위에 따라서도 데이터형이 다르기 때문이다.<br>
주로 정수는 int, 실수는 double을 쓰며,<br>
정수에서 수의 범위가 -147~147에 해당한다면 byte, -32768~32767에 해당한다면 short, -2147483648~2147483647에 해당한다면 int,<br>
더 크면 long(-9223372036854775808~9223372036854775807)을 쓰고, 실수에서는 int와 범위가 같으면 float, long과 범위가 같으면 double을 쓴다.<br><br>
당신은 a 라는 int형 변수를 만들기로 했다.<br>
이제 당신은 a 라는 변수에 어떤 값을 넣을 지를 정해야 한다.<br>
이를 '변수 초기화'라 한다.<br>
당신은 a 에 200이라는 정수를 넣었다고 하자.<br>
이제 당신은 변수를 마스터 했다!<br>
이거 하나만 더 알면.<br>
변수를 선언할때는 규칙이 있는데,<br>
첫째, 변수 이름에 숫자를 첫 글자로 쓸 수 없다(나머지 자리는 가능)<br>
둘째, 이름에는 공백이 들어갈 수 없다.<br>
셋째, 예약어(ex. int, long과 같은 이미 기능이 정해져 있는 단어)를 변수 이름으로 사용할 수 없다.<br>
넷째, 변수에는 영문자, 숫자, 달러기호, 또는 밑줄만 사용할 수 있다.<br>
개발자 사이에서는 암묵적인 관례가 있는데 그 이름은 바로 '낙타표기법'이다.
낙타표기법은 변수 이름이 여러 단어일때,
첫 번째 단어는 소문자로 시작하고 그 이후의 각 단어는 대문자로 시작하는 방법이다.
</h3>
