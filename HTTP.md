...
<h4>
하이퍼텍스트가 뭘까?<br><br>
하이퍼텍스트의 정의는 "사용자에게 비순차적인 검색을 할 수 있도록 제공되는 텍스트.<br><br>
문서 속의 특정 자료가 다른 자료나 데이터베이스와 연결되어 있어<br><br>
서로 넘나들며 원하는 정보를 얻을 수 있다. - 출처:네이버사전"으로,<br><br>
다른 사람들이 이걸 보면 "엥 이거 걍 하이퍼링크 아님?"이라고 할 수 있지만,<br><br>
하이퍼텍스트가 하이퍼링크보다 좀 더 큰 범위이다.<br><br>
하이퍼텍스트는 다른 문서나 위치로 이동할 수 있는 기능이 포함된 문서를 말하고,<br><br>
하이퍼링크는 누르면 다른 문서나 위치로 이동하는 실제 링크를 말한다.
</h4>
...
<h3>
HTTP Method에는 GET, POST, PUT, PATCH, DELETE가 있다.<br><br>
1. GET<br><br>
...
5. DELETE<br><br>
DELETE 메서드는 지정된 리소스를 삭제한다.<br><br>
데이터를 삭제하는 것이기 때문에 Body값과 Content-Type 값이 비워져있다.<br><br>
URL을 통해서 어떠한 데이터를 삭제할 것인지 받는다.<br><br>
데이터 삭제에 성공한다면 Body값 없이 성공 응답만 보내게 된다.<br><br>
</h3>
...
<h3>
HTTP Status Code는 특정 HTTP 요청이 성공적으로 완료되었는지 알려준다.<br><br>
응답은 5개 그룹으로 나누어지는데, 정보를 제공하는 응답, 성공적인 응답, 리다이렉트, 클라이언트 에러, 마지막 서버 에러 이다.<br><br>
대표적인 HTTP Status Code로는 200,404,500이 있다.<br><br>
